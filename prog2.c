...
int cycle(int s) {
    ...
    for(allV = 0; allV < (MAXPROC + MAXRSRC); allV++) {
        /* set v = 0 in all process and resource nodes */
        prn[allV].v = 0;
    }
    prn[s].v = 1; /* mark node p as having been visited */
    q = s; /* set q = p */

    for(q = prn[q].e;; q = prn[q].e) { /* loop B: set q = prn[q].e */
        if(q == -1) {
            /* no edge from previous node */
            break;
        } else {
            if(q == s) {
                /* returned to the starting node; have found a cycle starting with process s */
                return 1;
            }
            if(prn[q].v != 0) {
                /* have found a cycle, but it didn't start with p */
                break;
            } else {
                /* mark node q as having been visited (prn[q].v = 1); repeat from loop B */
                prn[q].v = 1;
                continue;
            }
        }
...
void putrcycle(int i) {
    ...
    r = prn[i].e; /* set R = rg[P].e */
    q = r; /* set Q = R */

    printf("    Resources ");
    while(printf("%d", q - MAXPROC + 1)) {
        q = prn[q].e;
        q = prn[q].e;
        if(q != r) {
            printf(", ");
...
int deadlock(void) {
    ...
    /*-------------------------------*/
    /* Construct the resource graph prn[] */
    /*-------------------------------*/
    for(i = 0; i < np; i++) {
        /* set process nodes; prn[i].e = MAXPROC + resource waiting for */
        if(proc[i + 1].state > 0) {
            prn[i].e = MAXPROC + proc[i + 1].state - 1;
        } else {
            prn[i].e = -1;
        }
    }
    ...
    x = 1;
    for(i = MAXPROC; i <= (MAXPROC + nr - 1); i++) {
        /* set resource nodes */
        prn[i].e = rstate[x] - 1;
        x++;
    }
    ...
    /*-----------------------------------------------------------------*/
    for(i = 0; i < np; i++) {
        /* check for a cycle starting at each process node */
